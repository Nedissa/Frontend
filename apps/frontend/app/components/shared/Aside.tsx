...
export function Aside({
  ...
  return (
    ...
    >
      ...
      {/* Mobil login: nedfälld panel från toppen (som Apohem). Mobil cart + övriga: full panel från höger. */}
      <aside
        className={
          type === 'login'
            ? 'flex xl:hidden w-full fixed left-0 right-0 top-0 bg-white shadow-lg z-50 flex-col'
            : `${type === 'cart' ? 'flex xl:hidden w-full md:w-1/2' : 'flex w-full'} fixed right-0 top-0 max-w-md bg-white shadow-lg z-50 flex-col ${expanded ? 'translate-x-0' : 'translate-x-full'}`
        }
        style={
          type === 'login'
            ? { transition: 'transform 300ms cubic-bezier(0.4, 0, 0.2, 1)', transform: expanded ? 'translateY(0)' : 'translateY(-100%)', maxHeight: '90dvh' }
            : { transition: 'transform 300ms cubic-bezier(0.4, 0, 0.2, 1)', height: '100dvh' }
        }
      >
